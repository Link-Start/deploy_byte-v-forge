...
Dashboard 模块、事件拓扑和 runtime/provider adapter 也由部署仓声明式校验：`dashboard-catalog.json` 必须指向服务拥有方的 `webui/src/dashboard/manifest.tsx`，`event-topology.json` 必须区分 JetStream 可回放事件和 NATS core hotstream，`runtime-adapter-catalog.json` 必须声明每个 provider/runtime adapter 的 SPI、注册点和遗留迁移债。
